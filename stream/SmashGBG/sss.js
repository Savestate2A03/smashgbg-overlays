var piio = new PiioConnector("sss");
var countdown = new Countdown();
	
const imageSlug = "NFP-SSS.png";
const staticLayoutPath = "/_resources/overlays/" + imageSlug;

var countdownTimerUpdater;
...
function updateTime(countdown) {
	document.getElementById('countdownText').insertValueResize(countdown.FullString);
}
...
piio.on("scoreboard", (data) => {
    ...
    const standbyTop = piio.getFieldValue("standby");
	const standbyTopText = `${standbyTop}`;
	document.getElementById("standbyTopText").insertValueResize(standbyTopText);

    const standbyBottom = piio.getFieldValue("standby2");
	const standbyBottomText = `${standbyBottom}`;
	document.getElementById("standbyBottomText").insertValueResize(standbyBottomText);
